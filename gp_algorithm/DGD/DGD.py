...
def get_optimized_prefix_embedding(model_name, orginal_emb_logits, new_logits, user_prompt, prompt_text, target_text, initial_ids, iterations, user_prompt_id, target_prompt_id, topk_result, model, tok, nconst_embedding, thredhold):
    ...
    for epoch in range(num_epochs):
        ...
        one_hot, loss_list, query_count = greedy_coordinate_gradient_step(model, tok, one_hot, embed_weights, input_embeddings, new_target_tokens, iterations, user_prompt_id, target_prompt_id, user_prompt, topk_result, nconst_embedding, count_limit, orginal_emb, k, Instructed_embeddings, user_prompt, thredhold, query_count)

        token_ids = torch.argmax(one_hot, dim=-1)
        optimized_prefix = tok.decode(token_ids[0].tolist(), skip_special_tokens=True)

    return token_ids, optimized_prefix, loss_list, query_count

# Define the GCG step
def greedy_coordinate_gradient_step(model, tok, one_hot, embed_weights, input_embeddings, new_target_tokens, iterations, user_prompt_id, target_prompt_id, user_prompt, topk_result, nconst_embedding, count_limit, orginal_emb, k, Instructed_embeddings, user_query, thredhold, query_count):
    ...
    for epoch in range(iterations):
        ...
        embed_weights = embed_weights.detach().to(dtype=torch.float16)
        ...
        ###############################统计查询次数################################
        top_k = get_relevant_documents(torch.stack(nconst_embedding), full_query, topk_result, model, tok)
        query_count = query_count + 1
        
        success = check_success(target_prompt_id, user_prompt_id, top_k)
        if success == 2 :
            return one_hot, loss_list, query_count
        ###########################################################################
        ...
        with torch.no_grad():
            ...
            one_hot = best_one_hot.detach().requires_grad_(True)
            ...
            if epoch % 20 == 0:
                ...
                '''print(one_hot.grad)
                print(combined_embeddings.grad)
                print(embed_weights.grad)
                print(prefix_embeddings.grad)
                print(input_embeddings.grad)'''
                Instructed_embeddings = Instructed_embeddings.detach()
    ...
    return one_hot_return, loss_list, query_count